...
export class Got {
  async getRandomQuote(): Promise<Quote> {
    return fetch('https://game-of-thrones-quotes.herokuapp.com/v1/random').then(response => {
      return response.json() as  Promise<Quote>
    })
  }

  async getHouse(house: string): Promise<House[]> {
    return fetch(`https://game-of-thrones-quotes.herokuapp.com/v1/house/${house}`).then(response => {
